...
class MY_Deeplearning():
    def __init__(self, data: pd.DataFrame, mode: str = 'Train', Target: str = ''):
        ...
        self.max_encoder_length = 24 # 입력으로 사용하고자 하는 데이터의 길이
        self.batch_size = 128
        ...
        self.model_path = self.time + "_" + self.target + "_IN_" + str(self.max_encoder_length) + "_OUT_" + str(self.max_prediction_length) + "pt"
    ...
    def Show_BL_performance(self):
        ...
        baseline_performance = (actuals - baseline_predictions).abs().mean().item()
    ...
    def Show_Model_performance(self):
        ...
        for x, (y, weight) in iter(self.val_dataloader):
            break
        actuals = y
        best_performance = (actuals - predictions).abs().mean().item()
    ...
    def Predict_new(self):
        ...
        self.prediction_x = prediction_x['decoder_time_idx'].tolist()[0]
        self.prediction_y = prediction_y.tolist()[0]
        return self.prediction_x, self.prediction_y
...
class MY_Blackbox():
    ...
    def __init__(self, data) -> None:
        ModelOpen = MY_Deeplearning(data = data, mode='Predict', Target='Open price')
        ModelOpen.Run(mode='Predict')
        ModelHigh = MY_Deeplearning(data = data, mode='Predict', Target='High price')
        ModelHigh.Run(mode='Predict')
        ModelLow = MY_Deeplearning(data = data, mode='Predict', Target='Low price')
        # ModelLow.Set_hyper_params(gradient_clip_val=0.15, hidden_size=32, lstm_layers = 2, dropout=0.2, hidden_continuous_size=16, attention_head_size=4, learning_rate=0.03)
        ModelLow.Run(mode='Predict')
        ModelClose = MY_Deeplearning(data = data, mode='Predict', Target='Close price')
        # ModelClose.Set_hyper_params(gradient_clip_val=0.15, hidden_size=32, lstm_layers = 2, dropout=0.2, hidden_continuous_size=16, attention_head_size=4, learning_rate=0.04)
        ModelClose.Run(mode='Predict')
        pass

    # Encoder 길이 만큼 Input 넣으면 Output이 나올 수 있도록 Setting

    # 평가 및 Back test function 추가




if __name__ == "__main__":
    df = pd.read_csv(r'C:\Programming\python\TotheMoon\PyOneDark\Data_Storage\4h\from200211_1200to220211_1200_4h.csv')
    data = df
