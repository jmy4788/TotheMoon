...
from PyQt6.QtCore import QPointF, QRectF, Qt, QPoint, QCoreApplication
# QPicture는 도화지 QPainter는 펜이라고 생각할 수 있음
from PyQt6.QtGui import QPainter, QPicture, QScreen, QPen, QColor, QBrush, QFont, QAction, QIcon, QTextDocument
from PyQt6.QtWidgets import QWidget, QMainWindow, QGraphicsObject, QApplication, QLabel, QGraphicsView, \
    QGraphicsScene, QGraphicsWidget, QGridLayout
import sys
...
class CurPos(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.label = []
        self.__is_first = True
        # 리스트 컴프리헨션으로 QLabel 10개 만들어 봤음
        self.label = [QtWidgets.QLabel() for x in range(10)]
    ...
    def first_row(self):
        ...
        for index, content in enumerate(first_row_list):
            self.layout.addWidget(QtWidgets.QLabel(f'{content}'), 0, index)
        return
...
if __name__ == '__main__':
    ...
    file_path = r'D:\재테크/'
    df = pd.read_csv(file_path + 'candle.csv')
    opens = df['Open']
    highs = df['High']
    lows = df['Low']
    closes = df['Close']
